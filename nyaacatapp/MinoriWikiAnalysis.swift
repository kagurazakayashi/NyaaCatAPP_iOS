...
class MinoriWikiAnalysis: NSObject {
    ...
    func 获取主菜单() -> Dictionary<String,[[String]]>? {
        ...
        let 分类区间:[String] = 主菜单区间.components(separatedBy: "<h3>")
        for 当前分类区间:String in 分类区间 {
            let 分类内容组:[String] = 当前分类区间.components(separatedBy: "</h3>")
            if (分类内容组.count == 2) {
