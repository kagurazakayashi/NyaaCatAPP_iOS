...
class StatusTVC: UITableViewController {
    ...
    func 装入数据() {
        ...
        if (要呈现的数据 == 呈现数据.玩家列表) {
            ...
            行主标题 = 在线玩家数据
//            行副标题 = nil
            默认头像 = UIImage(contentsOfFile: Bundle.main.path(forResource: "player_face", ofType: "png")!)!
            ...
            默认头像 = UIImage(contentsOfFile: Bundle.main.path(forResource: "home_icon", ofType: "png")!)!
            ...
            默认头像 = UIImage(contentsOfFile: Bundle.main.path(forResource: "signshop_icon", ofType: "png")!)!
        } else if (要呈现的数据 == 呈现数据.世界列表) {
            self.title = "世界列表"
            let 在线世界数据:[String] = 全局_综合信息!["世界列表"] as! [String]
            行主标题 = 在线世界数据
            行副标题 = nil
            默认头像 = UIImage(contentsOfFile: Bundle.main.path(forResource: "follow_on", ofType: "png")!)!
        }
        tableView.reloadData();
    }
    
    func 坐标修正(_ 坐标描述:String) -> String {
        let 坐标数组:[String] = 坐标描述.components(separatedBy: ",")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let 行:Int = (indexPath as NSIndexPath).row
        if (要呈现的数据 == 呈现数据.玩家列表) {
            cell.textLabel?.text = nil
            cell.detailTextLabel?.text = nil
            let 完整路径:String = "\(全局_喵窩API["32px头像接口"]!)\(图像路径![行])"
            cell.imageView?.setImageWith(URL(string: 完整路径)!, placeholderImage: 默认头像)
            ...
            let 格式:WKWebView = WKWebView(frame: CGRect(x: 50, y: 10, width: cell.frame.size.width - 50, height: cell.frame.size.height - 10), configuration: 浏览器设置)
            格式.backgroundColor = UIColor.clear
            格式.isUserInteractionEnabled = false
            格式.isOpaque = false
            cell.addSubview(格式)
