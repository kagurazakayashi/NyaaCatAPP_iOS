//
//  PrivateConstant.swift
//  nyaacatapp
//
//  Created by 神楽坂雅詩 on 16/3/19.
//  Copyright © 2016年 KagurazakaYashi. All rights reserved.
//  全局_喵窩API[""]

import UIKit

class PrivateConstant: NSObject {
    let PrivateConstantData:Dictionary<String,String> = [
        "动态地图域名":"https://map.nyaa.cat",
        "动态地图主页":"https://map.nyaa.cat/index.html",
        "动态地图登录接口":"https://map.nyaa.cat/up/login",
        "32px头像接口":"https://map.nyaa.cat/tiles/faces/32x32/",
        "消息发送接口":"https://map.nyaa.cat/up/sendmessage",
        "论坛地址":"https://bbs.nyaa.cat/?nojs=0",
        "测试动态地图用户名":"KagurazakaYukari",
        "测试动态地图密码":"Wangxh1993",
        "静态地图接口":"https://map.nyaa.cat/?worldname=world&mapname=flat&nogui=true",
        "注册页面标题":"Minecraft Dynamic Map - Login/Register",
        "地图页面标题":"NyaaCat Minecraft Map",
        "Ilse地图":"http://miz.audio/nyaamaps/",
        "关于和许可":"https://api.yoooooooooo.com:888/api/nyaacatapp/page/README/",
        "游戏直播":"https://api.yoooooooooo.com:888/nyaacatapp/page/LIVE/",
        "API域名":"https://api.yoooooooooo.com:888",
        "API路径":"https://api.yoooooooooo.com:888/nyaacatapp/index.html"]
}
