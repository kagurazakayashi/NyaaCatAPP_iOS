...
class MapVC: UIViewController , WKNavigationDelegate {
    ...
    func 左上按钮点击() {
        动态地图工具栏.isHidden = !动态地图工具栏.isHidden
        转到.isHidden = 动态地图工具栏.isHidden
    }
    func 右上按钮点击() {
        //TODO: 预留地图切换接口
...
//        选择地图.addAction(导航地图按钮)
        ...
        选择地图.addAction(取消按钮)
        self.present(选择地图, animated: true, completion: nil)
    }
    
    func 装入地图(_ x:String?, y:String?, z:String?) {
    ...
    func 动态地图工具栏可用(_ 是否可用:Bool) {
        左上按钮?.isEnabled = 是否可用
        转到.isHidden = !是否可用
        动态地图工具栏.isHidden = !是否可用
    }
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        ...
        消息提示.addAction(取消按钮)
        self.present(消息提示, animated: true, completion: nil)
    }
    ...
    func 装入网页(_ 网址:String) {
        ...
        let 要加载的网页URL:URL = URL(string: 网址)!
        let 加载地图网络请求:URLRequest =  URLRequest(url: 要加载的网页URL, cachePolicy: 全局_缓存策略, timeoutInterval: 30)
        浏览器!.load(加载地图网络请求)
    }
    
    @IBAction func 动态开关(_ sender: UISwitch) {
        装入地图(nil, y: nil, z: nil)
    }
    
    @IBAction func 三维开关(_ sender: UISwitch) {
        装入地图(nil, y: nil, z: nil)
    }
    
    @IBAction func 转到按钮(_ sender: UIButton) {
    ...
    func 提示框处理(_ 提示框:UIAlertController) {
